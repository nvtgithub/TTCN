        ...
        success: function (response) {
            ...
            if (cartHover_existItem.length) {
                ...
                var newItem =
                    ...
                    '<td class="si-text">\n' +
                    '  <div class="product-selected">\n' +
                    '        <h6 class="text-truncate" style="width: 170px;overflow: hidden;text-overflow: ellipsis;line-height: 25px;-webkit-line-clamp: 1;height: 25px;-webkit-box-orient: vertical;">' + response['cart'].name + '</h6>\n' + '<span> - '+ response['cart'].options['colorProduct'] + '</span>'
...
function removeCart(rowId, name) {
    $.ajax({
        type: "GET",
        url: "cart/delete",
        data: {rowId: rowId },
        success: function (response) {
            ...
            $('.subtotal-money').text(response['total'] + ' VNĐ');
            $('.cart-total-money').text(response['total'] + ' VNĐ');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (qty === 0) {
                ...
                cart_existItem.find('.total-price').text(new Intl.NumberFormat().format((response['cart'].price * response['cart'].qty).toFixed()) + ' VNĐ');
            }
